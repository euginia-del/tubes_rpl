...
$stmt = $db->prepare('
    SELECT 
        ...
        SUM(CASE WHEN status_order = "selesai" THEN harga_snapshot ELSE 0 END) as completed_revenue
    FROM orders 
...
// Get daily report for selected month
$stmt = $db->prepare('
    SELECT 
        ...
        status_order
    FROM orders 
    WHERE YEAR(tanggal_order) = ? AND MONTH(tanggal_order) = ?
    GROUP BY DATE(tanggal_order), status_order
    ORDER BY date DESC
');
...
// Get total stats
$stmt = $db->query('SELECT COUNT(*) as total_orders, SUM(harga_snapshot) as total_revenue FROM orders');
$totalStats = $stmt->fetch();

$stmt = $db->query('SELECT COUNT(*) as total_users FROM user WHERE role = "customer"');
$userCount = $stmt->fetch();

// Get pending payments count for supervisor
$stmt = $db->query('SELECT COUNT(*) as pending_payments FROM orders WHERE status_order = "pending"');
...
<div class="md:hidden bg-white dark:bg-slate-800 shadow-sm sticky top-0 z-40">
    <div class="flex items-center justify-between px-4 py-3">
        <a href="<?= $role === 'admin' ? 'admin.php' : 'supervisor.php' ?>" class="text-gray-600 dark:text-gray-300">
            <span class="material-symbols-outlined">arrow_back</span>
        </a>
        <span class="text-lg font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Reports</span>
...
<div class="md:ml-72">
    <div class="container-responsive py-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white">Financial Reports</h1>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-lg mb-6">
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                <div>
                    <label class="text-sm font-semibold text-gray-700 dark:text-gray-300">Year</label>
                ...
                <div>
                    <label class="text-sm font-semibold text-gray-700 dark:text-gray-300">Month</label>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg mb-6">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-4">Monthly Revenue <?= $year ?></h2>
            <canvas id="revenueChart" height="200"></canvas>
        </div>

        <!-- Service Popularity -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg mb-6">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-4">Service Popularity <?= $year ?></h2>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-4">Daily Report - <?= date('F Y', mktime(0,0,0,$month,1)) ?></h2>
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                        <tr class="border-b dark:border-slate-700">
                    ...
                    <tbody>
                        ...
                        foreach($dailyReports as $report):
                            $dailySum[$report['date']]['orders'] = ($dailySum[$report['date']]['orders'] ?? 0) + $report['orders'];
                            $dailySum[$report['date']]['revenue'] = ($dailySum[$report['date']]['revenue'] ?? 0) + $report['revenue'];
                        endforeach;
                        krsort($dailySum);
                        ?>
                        <?php foreach($dailySum as $date => $data): ?>
                        <tr class="border-b dark:border-slate-700">
                            <td class="py-3"><?= date('d F Y', strtotime($date)) ?></td>
                            <td class="text-center py-3"><?= number_format($data['orders']) ?></td>
                            <td class="text-right py-3 text-primary font-semibold">Rp <?= number_format($data['revenue'], 0, ',', '.') ?></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if(empty($dailySum)): ?>
                        <tr>
                            <td colspan="3" class="text-center py-8 text-gray-500">No data for this period</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
...
const months = <?php 
    ...
    foreach($monthlyReports as $report) {
        $monthNum = intval(substr($report['period'], 5));
        $monthlyData[$monthNum] = $report['total_revenue'];
    }
